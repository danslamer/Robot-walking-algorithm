...
uint16 CPWM[MOTOR_NUM]=   {1500,1500,1500,1500,1500,1500,1500,1500,1500};//位置
  ...
  InitMotor();//初始化舵机
	...
	{	
		...
		SendUartState();//发送状态信息
		 ...
		 scan_ps2();	
  #endif
		//进入判断函数(进行足部扫描)（sys定时器多次检测，一次异常加1，多次异常则判断为进入异常）->发生异常则进入中断->中断中进行判断哪种异常并处理
	//	LED_State();
	// 	Check_Power();
		  Check_key();
		//cur_count++;
...
void Check_key(void)
{
	...
	{
		flag_key=0;
		key_val=KEY_Scan();//返回值为1时有按键按下
		if(key_val==1)//有按键按下
		{
			key_val=0;
			memcpy(redata,key_buf,sizeof(key_buf));//C 库函数,从存储区 str2 复制 n 个字节到存储区 str1
			Beep_Test();
			flag_connect = 1;
			flag_RecFul = 1;
...
void Check_Power(void)
{
		...
		{
			flag_adc=0;
			voltemp=Get_Adc(4)*3.3*11*100/4096;	//电阻分压，具体根据原理图简单分析可以得到	这里将电池电压放大100倍 浮点型运算影响单片机效率
			volcount++;
			volall+=voltemp;
			if(volcount==100)
			{
				...
				vol=volall/100;
				volall=0;
				//UART_Put_Inf("vol:",vol); //将读取到ADC1 通道4的ADC值通过串口1打印出来
		    if(vol>=500 && vol<=600)    //电池电量低  7.4V锂电池这里当电池电压低于6V就算没电   这里之所以大于5V(500)是为了防止没有接电池 接USB时候误报 
				{
					Beep=~Beep;
				}
				else 
